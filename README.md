...
Additionally, it provides two commands to open the current file in a new window or an existing window. It closes the current tab and opens the file in the specific window, and copies all view-specific settings, such as bookmarks and highlights, into the new opened tab (WARNING: copying the settings into the new tab seems to be working only in MacOS, and depends on build +4078). However, if there are unsaved changes, you will be prompted to save them first.
